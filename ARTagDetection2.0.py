...
cap = cv2.VideoCapture('tags0.mp4')
    ...
    for tag1, tag2 in enumerate(corners):
        ...
        for x in range(0,200):
            for y in range(0,200):
                xp, yp, zp = np.matmul(H_inverse, [x, y, 1])
                if (int(yp/zp) < size[0] and int(yp/zp) > 0) and (int(xp/zp) < size[1] and int(xp/zp) > 0):
                    perspectiveImage[x][y] = binary[int(yp/zp)][int(xp/zp)]
        ...
        for x in range(0,200):
            for y in range(0,200):
                xp, yp, zp = np.matmul(H_inverse, [x, y, 1])
                if (int(yp/zp) < size[0] and int(yp/zp) > 0) and (int(xp/zp) < size[1] and int(xp/zp) > 0):
                    arTag[int(yp/zp)][int(xp/zp)] = lenaImage[x][y]
